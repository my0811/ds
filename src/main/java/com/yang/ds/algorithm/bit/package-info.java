 ...
 *
 * ps:重点，负数的表示如果正数的最大值用负数表示就是绝对值1，如果是最小值0，则负数表示全是1，其他都是在之间，所以会有个一个符号位是1
 * 后面全是0，也就是"-0"的情况是一个比正数多出来的表示方式，可以代替-128，所以负数多一个表示就是这么来的
 *
 * 结论:
 ...
 * 反码:数的补码反码是其本身，负数的反码是符号位保持不变，其余位取反(也就是绝对值取反)
 * 1. 数在计算机底层是以补码的形式存储的
